...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception {
        ...
        Assert.assertEquals("주문 가격은 가격 * 수량 ", 10000 * orderCount, order.getTotalPrice());
    ...
    private Member createMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("경기", "용인", "123-123"));
        em.persist(member);
        return member;
    }
    ...
    public void 상품주문_재고수량초과() throws Exception {
        //given
        Member member = createMember();
        Item item = createItem("JPA BOOK", 10000, 10);

        int orderCount = 11;
        //when
        orderService.order(member.getId(), item.getId(), orderCount);
        //then
        Assert.fail("재고 수량 부족 예외 발생");
    }
    ...
    public void 주문취소() throws Exception {
        ...
        assertEquals("주문이 취소된 상품은 재고가 증가 되어야 한다.", 10, item.getStockQuantity());
    }
